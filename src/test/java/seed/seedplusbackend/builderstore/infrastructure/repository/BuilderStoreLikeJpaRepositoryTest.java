...
@RepositoryTest
@DisplayName("BuilderStoreLikeJpaRepository")
class BuilderStoreLikeJpaRepositoryTest extends AbstractPostgresContainerTest {

  @Autowired private BuilderStoreLikeJpaRepository builderStoreLikeJpaRepository;
  ...
  @Test
  @DisplayName("가상 점포 좋아요를 저장하고 조회할 수 있다")
  void saveAndFindById_smokeTest() {
    User owner = userJpaRepository.save(UserFixture.generalActiveUser("like-owner@test.com"));
    User liker = userJpaRepository.save(UserFixture.generalActiveUser("like-user@test.com"));
    Region region = regionJpaRepository.save(RegionFixture.seoulGangnamYeoksamLegalDong());
    CommercialArea area =
        commercialAreaJpaRepository.save(CommercialAreaFixture.developedActive("LK상권"));
    Industry industry = industryJpaRepository.save(IndustryFixture.largeRoot("LK-IND", "기타"));
    BuilderStore builderStore =
        builderStoreJpaRepository.save(
            BuilderStoreFixture.publicBuilderStore(owner, region, area, industry));

    BuilderStoreLike saved =
        builderStoreLikeJpaRepository.save(
            BuilderStoreLike.builder().builderStore(builderStore).user(liker).build());

    assertThat(saved.getId()).isNotNull();
    assertThat(builderStoreLikeJpaRepository.findById(saved.getId())).isPresent();
  }
}
