...
public record KosisBusinessSurvivalCollectRequest(
    @Schema(description = "조회 시작 연도. latestYearCount와 함께 사용할 수 없습니다.", example = "2021")
        @Min(1900)
        @Max(2200)
        Integer startYear,
    @Schema(description = "조회 종료 연도. latestYearCount와 함께 사용할 수 없습니다.", example = "2022")
        @Min(1900)
        @Max(2200)
        Integer endYear,
  ...
  public KosisBusinessSurvivalCollectCommand toCommand() {
    return new KosisBusinessSurvivalCollectCommand(startYear, endYear, latestYearCount, force);
  }
}
