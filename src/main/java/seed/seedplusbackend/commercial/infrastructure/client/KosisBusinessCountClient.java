...
import seed.seedplusbackend.commercial.application.result.KosisBusinessCountItem;
...
@Slf4j
@Component
public class KosisBusinessCountClient implements KosisBusinessCountClientPort {

  private final RestClient restClient;
  private final ObjectMapper objectMapper;
  private final KosisBusinessCountOpenApiProperties properties;

  public KosisBusinessCountClient(
      @Qualifier("kosisBusinessCountRestClientBuilder") RestClient.Builder restClientBuilder,
  ...
  @Override
  public List<KosisBusinessCountRowResult> fetch(KosisBusinessCountCollectCommand command) {
    byte[] responseBytes;
    try {
      responseBytes =
          restClient
      ...
      throw new KosisBusinessCountApiRequestException(true, exception);
    }
    ...
    try {
      ...
      List<KosisBusinessCountApiResponse> response =
  ...
  private List<KosisBusinessCountRowResult> toValidResults(
      List<KosisBusinessCountApiResponse> response) {
    List<KosisBusinessCountRowResult> results = new ArrayList<>();
    int skippedCount = 0;

    for (KosisBusinessCountApiResponse row : response) {
      try {
        if (row == null) {
          throw new IllegalArgumentException("KOSIS 응답 행이 비어 있습니다.");
        }
    ...
    if (skippedCount > 0) {
      log.warn("KOSIS 산업별 기업수 응답에서 유효하지 않은 행을 제외했습니다. skippedCount={}", skippedCount);
    }
    return List.copyOf(results);
  }

  private KosisBusinessCountRowResult toResult(KosisBusinessCountApiResponse row) {
    ...
    if (KosisBusinessCountItem.fromItemId(row.itemId()).isEmpty()) {
      throw new IllegalArgumentException("KOSIS 산업별 기업수에서 지원하지 않는 항목입니다.");
    }

    BigDecimal count = new BigDecimal(row.value().replace(",", ""));
    if (count.signum() < 0) {
      throw new IllegalArgumentException("KOSIS 기업수는 음수일 수 없습니다.");
    }
  ...
  static KosisBusinessCountApiRequestException requestException(HttpStatusCode statusCode) {
    return new KosisBusinessCountApiRequestException(statusCode.is5xxServerError());
  }
}
