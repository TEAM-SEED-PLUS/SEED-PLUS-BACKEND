...
public class PublicDataBatchCommandFactory {
  ...
  private final SeoulCollectionTargets targets;
  ...
  private List<CommercialDataCollectCommand> storeCommands() {
    var districts =
        properties.storeSigunguCodes().isEmpty()
            ? targets.districts()
            : properties.storeSigunguCodes();
    requireTargets(districts, "DB 서울 시군구");
  ...
  private List<CommercialDataCollectCommand> cityCommands() {
    var areas = properties.cityAreas().isEmpty() ? targets.cityAreas() : properties.cityAreas();
    requireTargets(areas, "DB 실시간 도시데이터 장소");
    return areas.stream()
        .<CommercialDataCollectCommand>map(
